...
#define GOODIX_PINCTRL_STATE_SUSPEND "gt1x_pin_suspend"
#define GOODIX_PINCTRL_STATE_ACTIVE  "gt1x_pin_active"
...
#define GT1X_VDD_MIN_UV	2850000
#define GT1X_VDD_MAX_UV	2850000
#define GT1X_VIO_MIN_UV	1750000
#define GT1X_VIO_MAX_UV	1950000
...
static int gt1x_power_on(struct device *dev, bool on)
{
	int rc = 0;
	...
	if (on) {
		...
		if (regulator_count_voltages(vdd_ana) > 0) {
			rc = regulator_set_voltage(vdd_ana, GT1X_VDD_MIN_UV, GT1X_VDD_MAX_UV);
			if (rc) {
		...
		if (regulator_count_voltages(vcc_i2c) > 0) {
			rc = regulator_set_voltage(vcc_i2c, GT1X_VIO_MIN_UV, GT1X_VIO_MAX_UV);
			if (rc) {
		...
		if (regulator_count_voltages(vcc_i2c) > 0)
			regulator_set_voltage(vcc_i2c, 0, GT1X_VIO_MAX_UV);
			regulator_put(vcc_i2c);

		if (regulator_count_voltages(vdd_ana) > 0)
			regulator_set_voltage(vdd_ana, 0, GT1X_VDD_MAX_UV);
...
static int gt1x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = gt1x_power_on(&client->dev, 1);
	if (ret) {
...
*/
	...
	/*add device attributes added by ztemt 2015.01.01*/
